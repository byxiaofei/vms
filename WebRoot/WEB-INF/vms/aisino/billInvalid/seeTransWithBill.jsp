...
<head>
	...
	function revoke(){
		...
		if(reqSource == "billCancel"){
			submitAction(document.forms[0], "listBillCancel_HX.action?fromFlag=menu");
		}
	}
	</script>
...
<body onkeydow="enterkey(event)">
	...
	%>
		...
		<td align="left"><%=trans.getCustomerName()==null?"":trans.getCustomerName()%></td>
		...
		<td align="right"><%=NumberUtils.format(trans.getIncome(),"",2)%></td> <!-- 收入 -->
		<td align="right"><%=NumberUtils.format(trans.getTaxAmt().add(trans.getIncome()),"",2)%></td> <!-- 价税合计 -->
		<td align="center"><%=trans.getBalance()%></td>
		<td align="center"><%=DataUtil.getFapiaoTypeCH(trans.getFapiaoType())%></td>
		<td align="center"><%=DataUtil.getDataStatusCH(trans.getDataStatus(), "BILL")%></td>
	</tr>
	<%
				}
			}
