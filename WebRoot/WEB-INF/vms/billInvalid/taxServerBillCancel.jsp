...
<head>
	...
	function createBillCancel(){
		...
		function cancelBillSelver(){
			...
			$.ajax({url: 'billNomeCancelSelver.action',
					type: 'POST',
					data:{billId:billId},
					async:false,
					dataType: 'text',
		...
		function getBillCancelResult(ret,billId){
			...
			//alert(ret);
			$.ajax({
				url:'parseBillCancelResult.action',
					type: 'POST',
					async:false,
					...
					success: function(result){
							...
							//alert(aa);
								...
								alert("发票作废成功");
							submitAction(document.forms[0], "listBillCancel.action");//撤销	
...
<body onkeydow="enterkey(event)">
	...
	<tr>
		...
		<tr>
			...
			<td><select id="billCancelInfo.dataStatus" name="billCancelInfo.dataStatus" >
					<option value="">全部</option>
					<option value="15" <s:if test='billCancelInfo.dataStatus=="15"'>selected</s:if><s:else></s:else>>已作废</option>
					<option value="14" <s:if test='billCancelInfo.dataStatus=="14"'>selected</s:if><s:else></s:else>>作废已审核</option>
				</select>
			</td>
			<td >
	...
	<!-- <table id="tbl_query" cellpadding="1" cellspacing="0">
 	...
 	<table id="tbl_tools" width="100%" border="0">
		<tr>
			<td align="left">
			<a href="#" onClick="createBillCancel()"><img src="<c:out value="${bopTheme}"/>/themes/images/icons/icon1031.png"/>作废</a>
