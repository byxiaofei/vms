...
var bMoveable=true;
var strFrame;		
document.writeln('<iframe id=endDateLayer frameborder=0 width=162 height=189 style="position: absolute;  z-index: 9998; display: none"></iframe>');
strFrame='<style>';
...
strFrame+='</table></td></tr>';
strFrame+='  <tr ><td width=142 height=98 >';
...
strFrame+='    </table></td></tr>';
strFrame+='			 </table></td></tr></table></div><a id=\'LayTime\'></a>';
...
function setdayRight(tt,obj) // 主调函数
{
	if (arguments.length > 2){alert("对不起！传入本控件的参数太多！");return;}
	if (arguments.length == 0){alert("对不起！您没有传回本控件任何参数！");return;}
	var dads = document.all.endDateLayer.style;
	...
	var thei = tt.clientHeight;	// TT控件本身的高
	var tleft = tt.offsetLeft;	// TT控件的定位点宽
	var ttWidth = tt.clientWidth;// TT控件本身的宽

	var ttyp = tt.type;	// TT控件的类型

	while (tt = tt.offsetParent){ttop+=tt.offsetTop; tleft+=tt.offsetLeft;}
	dads.top = (ttyp=="image") ? ttop : ttop;
	dads.left = tleft+ttWidth+4;
	outObject = (arguments.length == 1) ? th : obj;
	outButton = (arguments.length == 1) ? null : th;	// 设定外部点击的按钮
...
// 计算控件中天数差，并将结果返回给指定控件
    function CountDays(obj1,obj2,obj3)// CountDays(大日期控件,小日期控件,接收值控件)
 {
 var m=DateDiff(document.getElementById(obj2).value,document.getElementById(obj1).value);
 document.getElementById(obj3).value=m;
  }
