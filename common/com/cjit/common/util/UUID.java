 ...
 * UUID represents a 128-bit value. <p> There exist different variants of these
 * global identifiers. The methods of this class are for manipulating the
 * Leach-Salz variant, although the constructors allow the creation of any
 * variant of UUID (described below). <p> The layout of a variant 2 (Leach-Salz)
 * UUID is as follows: The most significant long consists of the following
 * unsigned fields:
 * 
 * <pre>
 ...
 * variant. <p> The version field holds a value that describes the type of this
 * <tt>UUID</tt>. There are four different basic types of UUIDs: time-based,
 * DCE security, name-based, and randomly generated UUIDs. These types have a
 * version value of 1, 2, 3 and 4, respectively. <p> For more information
 * including algorithms used to create <tt>UUID</tt>s, see the Internet-Draft
 * <a
...
public final class UUID implements java.io.Serializable, Comparable{
	 ...
	 * <code>UUID</code> is generated using a cryptographically strong pseudo
	 ...
	 * number describes how this <tt>UUID</tt> was generated. The version
	 * number has the following meaning: <p> <ul> <li>1 Time-based UUID <li>2
	 * DCE security UUID <li>3 Name-based UUID <li>4 Randomly generated UUID
	 * </ul>
	 ...
	 * number describes the layout of the <tt>UUID</tt>. The variant number
	 * has the following meaning: <p> <ul> <li>0 Reserved for NCS backward
	 * compatibility <li>2 The Leach-Salz variant (used by this class) <li>6
	 * Reserved, Microsoft Corporation backward compatibility <li>7 Reserved
	 * for future definition </ul>
	...
	/**
	 * The timestamp value associated with this UUID. <p> The 60 bit timestamp
	 * value is constructed from the time_low, time_mid, and time_hi fields of
	 * this <tt>UUID</tt>. The resulting timestamp is measured in
	 * 100-nanosecond units since midnight, October 15, 1582 UTC. <p> The
	 * timestamp value is only meaningful in a time-based UUID, which has
	...
	/**
	 * The clock sequence value associated with this UUID. <p> The 14 bit clock
	 * sequence value is constructed from the clock sequence field of this UUID.
	 * The clock sequence field is used to guarantee temporal uniqueness in a
	 * time-based UUID. <p> The clockSequence value is only meaningful in a
	...
	/**
	 * The node value associated with this UUID. <p> The 48 bit node value is
	 ...
	 * guarantee spatial uniqueness. <p> The node value is only meaningful in a
	 * time-based UUID, which has version type 1. If this UUID is not a
	 * time-based UUID then this method throws UnsupportedOperationException.
	 ...
	 * <code>UUID</code>. <p> The UUID string representation is as described
	 * by this BNF :
	 * 
	 * <pre>
	...
	/**
	 * Compares this UUID with the specified UUID. <p> The first of two UUIDs
	 * follows the second if the most significant field in which the UUIDs
	...
	public int compareTo(Object val){
		...
		UUID obj = (UUID) val;
		return (this.mostSigBits < obj.mostSigBits ? -1
				: (this.mostSigBits > obj.mostSigBits ? 1
						: (this.leastSigBits < obj.leastSigBits ? -1
								: (this.leastSigBits > obj.leastSigBits ? 1 : 0))));
	}
