...
public class HomeDataXmlSerial{
	...
	private Document buildDocument(HomeDataDO homeDataDO){
		...
		if(homeDataDO != null){
			...
			if(items4Label != null){
				for(int i = 0; i < items4Label.size(); i++){
					...
					itemNode.addAttribute("sysid", item.getSysId());
			...
			for(int rowN = 0; rowN < rows4Tbody.size(); rowN++){
				...
				for(int colN = 0; colN < cells4Row.size(); colN++){
					...
					cellNode.addAttribute("sysid", cell.getSysId());
					cellNode.addAttribute("menuid", cell.getMenuId());
					if(cell.getKey() != null){
