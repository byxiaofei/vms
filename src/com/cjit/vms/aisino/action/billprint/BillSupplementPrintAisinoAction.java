...
public class BillSupplementPrintAisinoAction extends DataDealAction {
	...
	public String listBillSupplementPrint(){
		...
		try {
			...
			if ("menu".equalsIgnoreCase(fromFlag)) {
				...
				billCancelInfo.setFapiaoType("0");
				...
				billCancelInfo = new BillCancelInfo();
				// 开票日期(开始)
				if (!String.valueOf(this.request.getSession().getAttribute("billBeginDate")).isEmpty()
						&& !"null".equals(String.valueOf(this.request
								.getSession().getAttribute("billBeginDate")))) {
					this.billCancelInfo.setBillBeginDate(String.valueOf(this.request
							.getSession().getAttribute("billBeginDate")));
				}
				// 开票日期(结束)
				if (!String.valueOf(this.request.getSession().getAttribute("billEndDate")).isEmpty()
						&& !"null".equals(String.valueOf(this.request
								.getSession().getAttribute("billEndDate")))) {
					this.billCancelInfo.setBillEndDate(String.valueOf(this.request
							.getSession().getAttribute("billEndDate")));
				}
				// 客户名称
				if (this.request.getSession().getAttribute(
						"customerName")!=null&&!"".equals(this.request.getSession().getAttribute(
						"customerName"))) {
					this.billCancelInfo.setCustomerName(String.valueOf(this.request
							.getSession().getAttribute("customerName")));
				}
				// 是否手工录入
				if (!"null".equals(String.valueOf(this.request.getSession()
						.getAttribute("isHandiwork")))) {
					this.billCancelInfo.setIsHandiwork(String.valueOf(this.request
							.getSession().getAttribute("isHandiwork")));
				}
				// 开具类型
				if (!"null".equals(String.valueOf(this.request.getSession()
						.getAttribute("issueType")))) {
					this.billCancelInfo.setIssueType(String.valueOf(this.request
							.getSession().getAttribute("issueType")));
				}
				// 发票类型
				if (!"null".equals(String.valueOf(this.request.getSession()
						.getAttribute("fapiaoType")))) {
					this.billCancelInfo.setFapiaoType(String.valueOf(this.request
							.getSession().getAttribute("fapiaoType")));
				}
				// 状态
				if (!"null".equals(String.valueOf(this.request.getSession()
						.getAttribute("dataStatus")))) {
					this.billCancelInfo.setDataStatus(String.valueOf(this.request
			...
			if (StringUtil.isNotEmpty(flag) && DataUtil.AUDIT_BILL.equalsIgnoreCase(flag)) {
			...
			billCancelInfoList = billSupplementPrintService.findBillSupplementPrintInfoList(billCancelInfo,currentUser.getId(), paginationList);
	...
	public String seeTransForBillSupplement() {
		try {
			billCancelInfo.setBillId((String) this.request.getParameter("billId"));
	...
	public void billSupplementPrintToExcel() throws Exception {
		try {
			...
			this.getAuthInstList(lstAuthInstId);
			billCancelInfo.setLstAuthInstId(lstAuthInstId);
			billCancelInfoList = billSupplementPrintService.findBillSupplementPrintQuery(billCancelInfo);
	...
	public void  showSubLock() throws Exception {
		...
		result = registeredInfo + "|" + taxDiskInfo.getTaxDiskPsw() + "|"+ fapiaoType;
		printWriterResult(result);
	...
	/**
	 * 后台根据前台页面选中的billId组装传进OCX的字符串
	 * 注册码|发票类型(0:专,1:普)|发票数量|发票ID^发票代码^发票号码|   发票ID^发票代码^发票号码为循环域
	 * @return
	 */
	public String showOCXSubstring() {
		...
		List  bills = billInfoService.findBillInfoByIDFaPiaoType(ids,faPiaoType);
		BillInfo billInfo=(BillInfo)bills.get(0);
				//：注册码|发票类型(0:专,1:普)|发票数量|发票ID^发票代码^发票号码|   红色部分为循环域
		sbInner.append('0').append("^");
		sbInner.append(billInfo.getBillId()).append("^").append(billInfo.getBillCode()).append("^").append(billInfo.getBillNo());
