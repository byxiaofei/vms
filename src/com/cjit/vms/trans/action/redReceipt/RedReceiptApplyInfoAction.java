package com.cjit.vms.trans.action.redReceipt;
...
public class RedReceiptApplyInfoAction extends DataDealAction {
	...
	protected Map chargesList;
	private Map chanNelList;
	private Map billFreqList;
	...
	public void redReceiptBillToExcel() throws Exception {
		...
		try {
			...
			if (null != type && type.equals("redReceiptExcel")) {
				...
				response.setHeader("Content-type", "application/vnd.ms-excel");
				response.setHeader("Content-Disposition", name);
				redReceiptApproveToExcel1(os, list);
			} else {
	...
	public void redReceiptApproveToExcel(OutputStream os, List content)
		...
		Label header18 = new Label(17, 0, "保单号", JXLTool.getHeader());
		ws.addCell(header1);
		ws.addCell(header2);
		ws.addCell(header3);
		ws.addCell(header4);
		...
		for (int i = 0; i < content.size(); i++) {
			...
			map.put("insureId",billInfo.getInsureId());
	...
	private void setWritableSheetForRedReceiptApprove(WritableSheet ws, Map o,
		...
		Label cell18 = new Label(17, column, o.get("insureId").toString(), tempCellFormat);
		ws.addCell(cell1);
		ws.addCell(cell2);
		ws.addCell(cell3);
		ws.addCell(cell4);
	...
	/**与核心交互*/
	public void issueRedBillAccessCore() throws Exception{
			...
			if("".equals(errorMessage)){//校验完成没有错误则生成报文
				...
				if(transList.size()==1){
					for (Iterator it = transList.iterator(); it.hasNext();) {
						...
						if("0".equals(billInfo.getFapiaoType())){ //发票类型
							billDoc.addElement("FAPIAO_TYPE").addText("3");
						}else if("1".equals(billInfo.getFapiaoType())){
							billDoc.addElement("FAPIAO_TYPE").addText("4");
						}
				...
				if(XmlUtil.validateXML(requestXml)){
					logManagerService.writeLog(this.request, this.getCurrentUser(),
							serialno, "红票开具申请接口", "请求报文", requestXml, "1");
					log.info("红票开具请求核心接口"+requestXml);
					//与核心交互2-与核心交互					
					String responseXml = "";
					if("LSP".equals(billInfo.getDsouRce())){	//个险
	...
	//				String responseXml = str;
					...
					logManagerService.writeLog(this.request, this.getCurrentUser(),
							serialno, "红票开具申请接口", "响应报文", responseXml, "1");
					log.info("红票开具响应接口"+responseXml);
					//与核心交互3-解析返回报文
	...
	public String listRedReceiptApply() {
		...
		try {
			...
			redReceiptApplyInfo.setLstAuthInstId(lstAuthInstId);
			chanNelList = this.vmsCommonService.findCodeDictionary("CHANNEL_TYPE");
			billFreqList=this.vmsCommonService.findCodeDictionary("PAYMENT_FREQUENCY");
			chargesList=this.vmsCommonService.findCodeDictionary("CHARGES_TYPE");
			dsouRceList = this.vmsCommonService.findCodeDictionary("DATA_SOURCE");
	...
	private void createRedBill(String strOldBillId, String newBillId,
		...
		// 金额_人民币 合计
		BigDecimal amtCnySum = new BigDecimal("0.00");
		// 税额_人民币 合计
		BigDecimal taxAmtCnySum = new BigDecimal("0.00");

		for (int i = 0; i < transBillList.size(); i++) {
			transBill = (RedReceiptTransInfo) transBillList.get(i);
			// 金额_人民币 合计算出
			amtCnySum = amtCnySum.add(transBill.getIncomeCny());
			// 税额_人民币 合计算出
	...
	public Map getBillFreqList() {
		return billFreqList;
	}

	public void setBillFreqList(Map billFreqList) {
		this.billFreqList = billFreqList;
	}

	public Map getChargesList() {
		return chargesList;
	}

	public void setChargesList(Map chargesList) {
