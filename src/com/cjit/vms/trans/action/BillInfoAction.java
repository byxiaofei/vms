...
public class BillInfoAction extends BillAction {
	...
	/**
	 * 票据编辑界面，依所选机构加载我方信息
	 * 我日了狗了，居然有这种写法，把所有信息拼成一个字符串，然后再通过写servlet的方法传给jsp用js脚本拆分
	 * 请使用Json传递！请使用Json传递！请使用Json传递!重要的事情说三遍
	 *
	 */
	// FIXME:后期统一调用loadSelfInfo()方法
	...
	public void loadSelfInfo(){
		try {
			...
			org = organizationService.getOrganization(org);
			if (org != null) {
				sb.append(JSONObject.fromObject(org));
			}
			this.response.setContentType("text/html; charset=UTF-8");
			this.response.getWriter().print(sb.toString());
			this.response.getWriter().close();
